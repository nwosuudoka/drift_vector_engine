...
use crate::{FOOTER_SIZE, compression::wrapper::CompressionStrategy};
use byteorder::{LittleEndian, ReadBytesExt};
use fastbloom::BloomFilter;
use std::io::{self, Cursor, Read};

// Other imports ... use crate::compression...
...
impl SegmentReader {
    ...
    pub async fn open(uri: &str) -> io::Result<Self> {
        ...
        // ... Parse Footer (Same logic as before) ...
    ...
    // ... read_bucket (calls manager.read_at) ...
    // Note: read_bucket implementation I provided in previous turn already calls manager.read_at
    // No changes needed there if read_at signature matches.
    pub async fn read_bucket(&self, bucket_id: u32) -> io::Result<(Vec<u64>, Vec<Vec<f32>>)> {
        // ... existing logic ...
        // Ensure it uses self.manager.read_at()
        // (The implementation in my previous message did exactly this)
        ...
        let loc = self
            ...
            .buckets
            .get(&bucket_id)
            .ok_or(io::Error::new(io::ErrorKind::NotFound, "Bucket"))?;
        ...
        let blob = self
            .manager
            .read_at(loc.offset, loc.length as usize)
            .await?;

        // ... Decompression logic (same as before) ...
        // (Assuming imports are correct)
        use crate::compression::wrapper::{decompress_vectors, transpose_from_columns};

        // [Simplified for brevity - replicate the column reading loop]
